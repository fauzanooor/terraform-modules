...
resource "azurerm_network_interface" "buburtimor-nic" {
    ...
    ip_configuration {
        ...
        public_ip_address_id          = azurerm_public_ip.buburtimor-pip["${count.index}"].id
    }
}

resource "azurerm_public_ip" "buburtimor-pip" {
    count               = var.countnumber
    name                = "pip_${var.workload}_${var.project}_${count.index}"
    resource_group_name = var.resource_group_name
    location            = var.location
